...
def index(request):
    ...
    for t in topics:
        ...
        if (j % topic_column_number) == 0:
            ...
            j = 0
    list_of_topics.append(entry)
    return render(request, 'subapp_gallery/index.html', {'topics': topics, 'list_of_topics': list_of_topics})
...
def topic(request, topic_name):
    ...
    for i in images:
        ...
        if (j % image_column_number) == 0:
            ...
            j = 0
    list_of_images.append(entry)
    return render(request, 'subapp_gallery/topic.html', {'topic': topic, 'images': images, 'list_of_images': list_of_images})


def detail(request, topic_name, photo_id):
    photo = get_object_or_404(Photo, topic__name=topic_name, id=photo_id)
    return render(request, 'subapp_gallery/detail.html', {'photo': photo})
